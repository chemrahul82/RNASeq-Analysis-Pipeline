...
def get_info_stat():
			...
			if os.path.exists('%s/%s/%s' %(projDir, isam, itim)):
				for irun in runs:
					if os.path.exists('%s/%s/%s/%s/%s.stats.txt' %(projDir, isam, itim, irun,prefix)):
						...
						#coverage of genes linked to pathogenesis of PNET
						fusionGenes = ['EWSR1','FLI1','ERG','ETV1','E1AF','FEV']
						for genes in fusionGenes:
							command = 'grep -P  \"^%s\t\" %s/%s/%s/%s/%s.genereads.xls' %(genes,projDir,isam,itim,irun,prefix)
							#command = 'awk -F, \'$0 == %s\t\' %s/%s/%s/%s/%s.genereads.xls' %(genes,projDir,isam,itim,irun,prefix)
							#print isam,irun
							os.system(command)  
	...
	wb = xlsxwriter.Workbook('PNET-RNA-Seq-Metric-Summary-Table-'+currtime.strftime('%Y-%m-%d-%H-%M-%S')+'.xlsx')
